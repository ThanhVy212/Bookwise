...
const StudentCard = ({ user }: StudentCardProps) => {
  ...
  return (
    <div className="relative w-full max-w-[540px]">
      {/* Top Lanyard Clip */}
      <div className="relative mx-auto -mb-3 flex h-9 w-16 items-center justify-center rounded-t-lg bg-[#2D3348] shadow-md z-10 border-t border-x border-light-100/10">
        <div className="h-2.5 w-7 rounded-full bg-dark-100/90 shadow-inner" />
      </div>

      {/* Main Badge Card */}
      <div className="relative flex flex-col gap-8 rounded-3xl bg-[#171B26] p-8 sm:p-10 border border-light-100/10 shadow-2xl">
        {/* User Info Header */}
        <div className="flex items-center gap-6">
          ...
          <div className="flex flex-col gap-1.5">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-sm font-medium text-[#FFE1BD]">
                {user.status === "APPROVED" ? "Verified Student" : user.status || "N/A"}
              </span>
            </div>

            <h2 className="text-3xl font-bold text-white">{user.fullName}</h2>
            <p className="text-base text-light-100 break-all">{user.email}</p>
          </div>
        </div>

        {/* University Name */}
        <div>
          <p className="text-sm font-semibold uppercase tracking-wider text-light-100/70">
            University
          </p>
          <p className="text-xl font-bold text-white mt-1">JS Mastery Pro</p>
        </div>
        ...
        <div>
          ...
          <p className="text-3xl font-extrabold text-white tracking-wider mt-1">
            {user.universityId}
          </p>
        </div>
...
const StylizedUniversityCard = ({
  ...
  return (
    <div className="relative p-5 sm:p-6 text-white flex flex-col justify-between h-56 sm:h-64 bg-gradient-to-r from-[#111A2E] via-[#19273C] to-[#0F1829]">
      {/* Background patterns */}
